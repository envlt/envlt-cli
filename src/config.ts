...
function storageReadError(cause: unknown): AppError {
  return new AppError(ErrorCode.STORAGE_READ_ERROR, 'Failed to read config from storage.', cause);
}

function storageWriteError(cause: unknown): AppError {
  return new AppError(ErrorCode.STORAGE_WRITE_ERROR, 'Failed to write config to storage.', cause);
}
...
export async function readConfig(
  ...
  let existsResult: Result<boolean>;
  try {
